...
public class HexMapPanel extends JPanel {
    ...
    public HexMapPanel() {
        ...
        // Dodaj bohaterów
        heroes.add(new Hero(getHex(0, 0), Color.BLUE));
        heroes.add(new Hero(getHex(2, -2), Color.RED));
        	            ...
        	            for (Hero other : new ArrayList<>(heroes)) {
        	                if (other != active && other.isAlive()) {
        	                    ...
        	                    if (clickedHex.equals(other.getPosition()) || dist == 1) {
        	                        other.damage(30); // zadaj obrażenia
        	                        if (!other.isAlive()) {
        	                            heroes.remove(other);
        	                        }
        	                        repaint();
    ...
    private void generateMap() {
        ...
        for (int q = -MAP_RADIUS; q <= MAP_RADIUS; q++) {
            for (int r = Math.max(-MAP_RADIUS, -q - MAP_RADIUS); r <= Math.min(MAP_RADIUS, -q + MAP_RADIUS); r++) {
                ...
                if (t < 10) hex.setTerrain(TerrainType.WATER);
                else if (t < 20) hex.setTerrain(TerrainType.MOUNTAIN);

                // Losowy obiekt
                int o = rand.nextInt(100);
                if (o < 5) hex.setObject(MapObject.CASTLE);
                else if (o < 10) hex.setObject(MapObject.MINE);
